...
const lastCommand = ref<{
    ...
    value: any;
    timestamp: number;
    origin: "schedule" | "management" | "device";
} | null>(null);
...
const getCommandHighlightClass = (
...
const deviceStatus = computed(() =>
    devicesStore.getDeviceStatus(deviceId.value)
);
...
const connectionSteps = ref([
    ...
    },
    {
        id: "ready",
        label: `Oda ${
            selectedRoom.value?.name || "X"
        } uzaktan izlenme ve yönetime hazır`,
...
const connectToIoT = () => {
    ...
    iotSocket.value.onopen = () => {
        ...
        setTimeout(() => {
            console.log(
                "[IoT WS] Device connected, schedule will be applied automatically if configured"
            );
        }, 3000);
    ...
    iotSocket.value.onmessage = (event) => {
        ...
        const origin: "schedule" | "management" | "device" =
            meta === "schedule"
                ? "schedule"
                : meta === "management"
                ? "management"
                    ...
                    if (origin !== "device") {
                        toast.add({
                            ...
                            summary:
                                origin === "schedule"
                                    ? "Çizelge Komutu"
                                    : "Yönetim Paneli Komutu",
                            detail: `Elektrik ${
                                newElectricityStatus ? "açıldı" : "kapatıldı"
                            }`,
                            life: 3000,
                        });
                    }
                        ...
                        toast.add({
                            ...
                            summary:
                                origin === "schedule"
                                    ...
                                    : "Yönetim Paneli Komutu",
                            detail: `Isıtma ${
                                newHeatingStatus ? "açıldı" : "kapatıldı"
                            }`,
                            life: 3000,
                        });
                    }
                        ...
                        toast.add({
                            ...
                            summary:
                                origin === "schedule"
                                    ...
                                    : "Yönetim Paneli Komutu",
                            detail: `Işık ${
                                newLightStatus ? "açıldı" : "kapatıldı"
                            }`,
                            life: 3000,
                        });
                    }
...
const sendTemperature = (value?: number) => {
    if (iotSocket.value && isConnected.value) {
        const tempToSend =
            typeof value === "number" ? value : temperature.value;
...
const updateTemperature = (
    ...
    source: "manual" | "simulation" = "manual"
) => {
    ...
    >
        ...
        <div class="max-w-6xl mx-auto">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800 mb-2">
                            Akıllı Bina Yönetim Sistemi - Simülasyon
                        </h1>
                        <p
                            class="text-gray-600"
                            v-if="selectedRoom"
                        >
                            {{ selectedRoom.name }} - Kat
                            {{ selectedRoom.floor }}, Oda
                            {{ selectedRoom.doorNumber }}
                        </p>
                    </div>
            ...
            >
                ...
                <div
                    class="relative bg-gradient-to-b from-blue-100 to-gray-100 rounded-lg p-8 min-h-[400px] overflow-hidden"
                >
                    <!-- Room Background -->
                    <div
                        class="absolute inset-0 bg-gradient-to-br from-sky-200 via-blue-50 to-gray-100"
                    ></div>

                    <!-- Window -->
                    <div
                        class="absolute top-4 right-4 w-32 h-48 bg-gray-300 rounded-lg border-4 border-gray-400 shadow-lg"
                    ...
                    >
                        ...
                        >
                            ...
                            <div
                                v-else
                                class="text-blue-800 text-xs font-bold"
                            >
                                AÇIK
                            </div>
                        </div>
                        <!-- Window frame -->
                        <div
                            class="absolute inset-2 border-2 border-gray-500"
                        ></div>
                    </div>
                    ...
                    >
                        <div class="relative">
                            ...
                            <div
                                ...
                                :class="
                                    lightStatus
                                        ? 'bg-yellow-300 shadow-[0_0_30px_20px_rgba(255,255,0,0.6)]'
                                        : 'bg-gray-200'
                                "
                            >
                                <div
                                ...
                                >
                                    ...
                                    >
                                        <path
                                            d="M12 2C8.13 2 5 5.13 5 9c0 2.38 1.19 4.47 3 5.74V17c0 .55.45 1 1 1h6c.55 0 1-.45 1-1v-2.26c1.81-1.27 3-3.36 3-5.74 0-3.87-3.13-7-7-7zm0 14h-1v-1h2v1zm0-4h-1v-1h2v1zm0-4h-1V7h2v1z"
                                        />
                                    </svg>
                                </div>
                                <div
                            ...
                            <div
                                v-if="lightStatus"
                                class="absolute top-full left-1/2 transform -translate-x-1/2 w-32 h-32"
                            >
                                <div
                    ...
                    <div class="absolute bottom-8 left-8">
                        <div class="relative">
                            <div
                                ...
                                :class="
                                    heatingStatus
                                        ? 'bg-red-600 shadow-[0_0_20px_10px_rgba(239,68,68,0.5)]'
                                        : ''
                                "
                            >
                                <div
                                ...
                                >
                                    <div
                                        class="w-2 h-2 bg-red-400 rounded-full animate-ping"
                                    ></div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="absolute bottom-8 right-8">
                        <div class="relative">
                            <div
                                ...
                                :class="
                                    electricityStatus
                                        ? 'bg-yellow-400 border-yellow-600 shadow-[0_0_25px_15px_rgba(234,179,8,0.6)]'
                    ...
                    >
                        <div
                            class="bg-white/90 backdrop-blur-sm rounded-lg p-4 shadow-xl border-2 border-blue-300"
                        >
                            <div class="text-center">
                                <div
                                    class="text-4xl font-bold text-blue-600 mb-1"
                                >
            ...
            >
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="flex items-center gap-4">
                        <ToggleSwitch
                            ...
                            @update:modelValue="sendWindowStatus"
                        />
                        <span
                            class="text-lg font-medium"
                            :class="
                                windowStatus ? 'text-green-600' : 'text-red-600'
                            "
                        >
                ...
                >
                    ...
                    <div class="flex items-center gap-4">
                        <ToggleSwitch
                            v-model="electricityStatus"
                            @update:modelValue="sendElectricityStatus"
                        />
                        <span
                            class="text-lg font-medium"
                            :class="
                ...
                >
                    ...
                    <div class="flex items-center gap-4">
                        <ToggleSwitch
                            v-model="heatingStatus"
                            @update:modelValue="sendHeatingStatus"
                        />
                        <span
                            class="text-lg font-medium"
                            :class="
                ...
                >
                    ...
                    <div class="flex items-center gap-4">
                        <ToggleSwitch
                            v-model="lightStatus"
                            @update:modelValue="sendLightStatus"
                        />
                        <span
                            class="text-lg font-medium"
                            :class="
                                lightStatus ? 'text-green-600' : 'text-red-600'
                            "
                        >
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="space-y-2 text-gray-600">
                        <div>
                            <span class="font-semibold">İsim:</span>
                            {{ selectedRoom.name }}
                        </div>
                        <div>
                        ...
                        <div>
                            <span class="font-semibold">Oda No:</span>
                            {{ selectedRoom.doorNumber }}
                        </div>
                        <div>
                            <span class="font-semibold">Sektör:</span>
                            {{ selectedRoom.sector }}
                        </div>
                        <div>
                            <span class="font-semibold">Cihaz ID:</span>
                            {{ deviceConnectedId }}
                        </div>
                    </div>
                </div>
        ...
        >
            <div class="space-y-4">
                ...
                <div
                    v-if="roomsByFloor[selectedFloor]"
                    class="grid grid-cols-5 gap-2 max-h-96 overflow-y-auto"
                >
                    <div
